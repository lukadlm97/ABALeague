...
{
    public class GetTeamStatsByLeagueIdQueryHandler :
        IQueryHandler<GetTeamStatsByLeagueIdQuery, Maybe<TeamStatsByLeagueDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetTeamStatsByLeagueIdQueryHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }
        public async Task<Maybe<TeamStatsByLeagueDto>> 
            Handle(GetTeamStatsByLeagueIdQuery request, CancellationToken cancellationToken)
        {
            ...
            if (league == null)
            {
                return Maybe<TeamStatsByLeagueDto>.None;
            }

            List<StatsItemByLeagueDto> list = new List<StatsItemByLeagueDto>();
            foreach (var item in rosterItems.Where(x => x.LeagueId == request.LeagueId).Select(x=>x.TeamId).Distinct())
            {
                var rosterItemsByTeamIds = rosterItems
                    .Where(x => x.TeamId == item && x.LeagueId == request.LeagueId)
                    .Select(x=>x.Id)
                    .ToList();

                var selectedBoxscores = boxscores.Where(x => rosterItemsByTeamIds.Contains(x.RosterItemId)).ToList();
                var team = teams.FirstOrDefault(x=>x.Id == item);
                if(team == null)
                {
                    continue;
                }
                ...
                list.Add(new StatsItemByLeagueDto(team.Id,
                    team.Name,
            ...
            return new TeamStatsByLeagueDto(league.Id, league.OfficalName, list.OrderByDescending(x => x.Points)
